...
mongoose.connect('mongodb://127.0.0.1:27017/user_management')
  .then(async () => {
    ...
    if (firstUser && firstUser.username !== 'Charitha123') {
...
app.get('/add-user', async (req, res) => {
  try {
    const existingUser = await User.findOne({ username: 'Charitha123' });
    if (existingUser) return res.status(400).send('User already exists');

    const newUser = new User({
      username: 'Charitha123',
      email: 'Charitha@example.com',
      password: 'securepassword',
      roles: ['admin'],
      profile: { firstName: 'Charitha', lastName: 'C', age: 25 }
    });
